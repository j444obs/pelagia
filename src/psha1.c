...
/* plg_SHA1Init - Initialize new context */
...
void plg_SHA1Update(SHA1_CTX* context, const unsigned char* data, unsigned int len)
{
    ...
    if ((j + len) > 63) {
        ...
        plg_SHA1Transform(context->state, context->buffer);
        for ( ; i + 63 < len; i += 64) {
            plg_SHA1Transform(context->state, &data[i]);
        }
        j = 0;
    }
    else i = 0;
...
#endif
    c = 0200;
    plg_SHA1Update(context, &c, 1);
    while ((context->count[0] & 504) != 448) {
	c = 0000;
        plg_SHA1Update(context, &c, 1);
    }
    plg_SHA1Update(context, finalcount, 8);  /* Should cause a plg_SHA1Transform() */
    for (i = 0; i < 20; i++) {
...
int plg_sha1Test(int argc, char **argv)
{
    ...
    plg_SHA1Init(&ctx);
    for(i=0;i<1000;i++)
        plg_SHA1Update(&ctx, buf, BUFSIZE);
    plg_SHA1Final(hash, &ctx);
